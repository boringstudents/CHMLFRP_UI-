...
class MainWindow(QMainWindow):
    ...
    def check_and_download_files(self):
        ...
        required_files = [
            ...
            get_absolute_path("frps_full.ini")
        ]
            ...
            try:
                ...
                with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                    for file in zip_ref.namelist():
                        if file.endswith(('frpc.exe', 'frps_full.ini')):
                            extracted_folder = file.split('/')[0]
                            zip_ref.extract(file)

                if extracted_folder:
                    for file in ('frpc.exe', 'frps_full.ini'):
                        src = os.path.join(extracted_folder, file)
                        dst = get_absolute_path(file)
                        if os.path.exists(src):
                            if os.path.exists(dst):
                                os.remove(dst)  # 如果文件已存在，先删除
                            shutil.move(src, dst)
                            self.logger.info(f"成功移动文件: {file}")
